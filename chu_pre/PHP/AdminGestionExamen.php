...
<html>
    <head>
        ...
        <title>CHU - Gestion Examens</title>
    </head>
    <body>
        <div class="wrapper">
            <div class="section">
            ...
            <div class="body-content">
                <div class="body">
                    <span id="titre">Listes des examens</span>
                    <span id="ajout"><span>+</span> <a href="NouveauExamen.php">Nouveau Examen</a></span>
                </div>
                <div class="content">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        ...
                        <?php
                                        ...
                                        $resultat = mysqli_query($con ,"SELECT * FROM examens"); 
                                        if($resultat) 
                                        { 
                                            while($ligne = mysqli_fetch_object($resultat))
                                            {
                                                    $labo = $ligne->id_labo;
                                                    $tube = $ligne->id_tube;
                                                    $res  = mysqli_query($con ,"SELECT code_L FROM labo WHERE id_labo='$labo'");
                                                    if($res)
                                                    {
                                                        while($ligne_l = mysqli_fetch_object($res))
                                                        {
                                                            $res_t  = mysqli_query($con ,"SELECT code_tube FROM tubes WHERE id_tube='$tube'");
                                                            if($res_t)
                                                            {
                                                                while($ligne_t = mysqli_fetch_object($res_t))
                                                                {
                                                            echo '<tr>
                                                            <td>'. $ligne->code_E . ' </td>
                                                            <td>'. $ligne->description_E .'</td>
                                                            <td>'. $ligne_l->code_L .'</td>
                                                            <td>'. $ligne_t->code_tube . ' </td>
                                                            <td class="icons">
                                                                <a href="ModifierExamen.php?id='.$ligne->code_E.'" class="update"><i class="bi bi-pencil-square"></i></a>
                                                                <a href="SupprimerExamen.php?id='.$ligne->code_E.'" ><i class="bi bi-trash delete"></i></a>
                                                            </td>
                                                                </tr>';
                                                        }
                                                    }
